...
class AddForm extends Component {
  ...
  onSubmit = (event) => {
    ...
    // const res = await axios.post("http://localhost:5000/student/create",{
      async function myFunction(state) {
        try {
          console.log("a");
          const res = await CallApi("student/create", "POST", {
            msv: state.msv,
            name: state.name,
            ...
            sum_of_credits: state.sum_of_credits,
            gpa: state.gpa,
            status: state.status,
            lop: state.lop,
          });
          console.log(res);
          sessionStorage.setItem("lop", res.data.newLop);
        } catch (error) {
          console.log(error); 
        }
      }
